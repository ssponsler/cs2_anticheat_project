...
// go run ./context-parser/wallpen_events.go -demo {.dem}

// ML format - JSONL/CSV
// go run ./context-parser/wallpen_events.go -demo {.dem} -out-format both -jsonl-out ./wallpen_events.jsonl -csv-out ./wallpen_events.csv
func main() {
	args := parseArgs()
	...
	fmt.Printf("\nTotal wall-penetration kills: %d\n", wallPenKillCount)
	if jsonlWriter != nil {
...
func parseArgs() cliArgs {
	...
	jsonlOut := flag.String("jsonl-out", "wallpen_events.jsonl", "Path to JSONL output file")
	csvOut := flag.String("csv-out", "wallpen_events.csv", "Path to CSV output file")
	flag.Parse()
...
func (w *jsonlWriter) Write(record wallPenRecord) error {
...
func (w *csvWriter) Write(record wallPenRecord) error {
